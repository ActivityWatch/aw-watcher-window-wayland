...
use wayland_client::{Connection, globals::registry_queue_init, delegate_noop, EventQueue};
        ...
        let timestamp = match self.is_afk {
            ...
            false => {
                let last_guaranteed_activity =
                    now - Duration::milliseconds(self.timeout_ms as i64);
...
pub fn init_afk_state(conn: &Connection,
                           timeout_ms: u32) -> anyhow::Result<(AfkState, EventQueue<AfkState>)> {
    ...
    let idle: OrgKdeKwinIdle = globals
                            .bind(&queue.handle(), 1..=OrgKdeKwinIdle::interface().version, ())?;

    let kwin_idle_timeout = 
        idle.get_idle_timeout(&seat, timeout_ms, &queue.handle(), ());
